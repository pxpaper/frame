...
def start_advertisement():
    ...
    try:
        ...
        # Prepend "PX" to the serial.
        mfg_data = bytearray("PX" + serial, 'utf-8')
        # We assume adapter index 0; adjust if needed.
        ad = advertisement.Advertisement("peripheral", 0)
        ad.local_name = "PixelPaper"
        ad.manufacturer_data = {0xFFFF: mfg_data}  # Use 0xFFFF as a sample manufacturer ID.
        ad.service_UUIDs = [PROVISIONING_SERVICE_UUID]
        ad.register()
        log_debug("Advertisement registered with manufacturer data: PX" + serial)
    except Exception as e:
...
if __name__ == '__main__':
    ...
    # Start the advertisement with the separate serial in manufacturer data.
    start_advertisement()
