...
• Sends AUTH_FAIL / OK via STATUS characteristic so the phone app knows
"""
...
fail_count        = 0           # updated by update_status / auth failures
...
status_char = None                # set in start_gatt_server()
...
SPIN_DELAY = 40  # ms (≈2× normal GIF speed)
    ...
    for idx in count():
        try:
            spinner_frames.append(
                tk.PhotoImage(file=SPINNER_GIF, format=f"gif -index {idx}")
            )
        except tk.TclError:
            break
...
def check_wifi_connection() -> bool:
    """Ping 8.8.8.8 once via TCP to see if Wi-Fi and routing work."""
    try:
...
def send_status(txt: str):
    """Notify phone app via STATUS characteristic."""
    if status_char:
        status_char.set_value(list(txt.encode()))
        status_char.notify()

def clear_wifi_profiles():
    """Delete every Wi-Fi profile and restart NM."""
    try:
...
def handle_wifi_data(payload: str):
    """Add NM profile, attempt connection, and send OK | AUTH_FAIL."""
    global fail_count
    try:
    ...
    # Delay verdict to let WPA handshake run
    def final_verdict():
        nonlocal ssid
        if check_wifi_connection():
            ...
            subprocess.run(["nmcli", "connection", "delete", ssid],
                           check=False)
            hide_spinner()
            status_label.configure(text="Wi-Fi authentication failed — wrong password?")
            send_status("AUTH_FAIL")
            fail_count = -999

    root.after(6000, final_verdict)
...
def handle_orientation_change(arg: str):
    ...
    pth = os.path.expanduser("~/.config/kanshi/config")
    os.makedirs(os.path.dirname(pth), exist_ok=True)
    with open(pth, "w") as f: f.write(cfg)
    os.chmod(pth, 0o600)
    subprocess.run(["killall", "kanshi"], check=False)
    subprocess.Popen(["kanshi", "-c", pth], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    log_debug("Portrait" if arg in ("90", "270") else "Landscape")

def ble_callback(val, _opt):
